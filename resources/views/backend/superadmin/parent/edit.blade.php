<form method="POST" class="d-block ajaxForm" action="{{ route('parent.update', $user->id) }}">
        @csrf
        @method('PATCH')
        <div class="form-row">
            <div class="form-group col-md-12">
                <label for="name">{{ translate('parent_name') }}</label>
            ...
            <div class="form-group col-md-12">
                ...
                <select name="gender" id="gender" class="form-control" required>
                    <option value="male" @php if($user->gender == 'male') echo 'selected'; @endphp>{{ translate('male') }}</option>
                    <option value="female" @php if($user->gender == 'female') echo 'selected'; @endphp>{{ translate('female') }}</option>
                    <option value="others" @php if($user->gender == 'others') echo 'selected'; @endphp>{{ translate('others') }}</option>
                </select>
            ...
            <div class="form-group  col-md-12">
                <button class="btn btn-block btn-primary" type="submit">translate('update_parent')</button>
            </div>
        </div>
    </form>
